...
, m_xml_file_name()
{
  ...
  m_gl_widget->setMovieFPS( unsigned( m_fps_spin_box->value() ) );
...
void ContentWidget::openScene()
{
  ...
  if( file_exists )
  {
    ...
    if( successfully_loaded )
    {
      ...
      m_fps_spin_box->setValue( int( fps ) );
...
void ContentWidget::openScene( const QString& scene_file_name )
{
  ...
  if( file_exists )
  {
    ...
    bool render_at_fps;
    bool lock_camera;
    const bool successfully_loaded{ m_gl_widget->openScene( scene_file_name, fps, render_at_fps, lock_camera ) };
...
{
  assert( m_gl_widget != nullptr );
  m_gl_widget->centerCamera( false );
}
...
{
  m_gl_widget->setMovieFPS( unsigned( fps ) );
}
