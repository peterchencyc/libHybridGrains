...
NearEarthGravityForce::NearEarthGravityForce(const Vector2s &g) : m_g(g) {}

NearEarthGravityForce::NearEarthGravityForce(std::istream &input_stream)
    : m_g(MathUtilities::deserialize<Vector2s>(input_stream)) {}

scalar NearEarthGravityForce::computePotential(const VectorXs &q,
  ...
  for (unsigned bdy_idx = 0; bdy_idx < nbodies; ++bdy_idx) {
    assert(masses(3 * bdy_idx + 0) == masses(3 * bdy_idx + 1));
    assert(masses(3 * bdy_idx + 0) > 0.0);
    U += -masses(3 * bdy_idx) * m_g.dot(q.segment<2>(3 * bdy_idx));
  }
  return U;
}

void NearEarthGravityForce::computeForce(const VectorXs &q, const VectorXs &v,
                                         const SparseMatrixsc &M,
                                         VectorXs &result) const {
  assert(q.size() % 3 == 0);
