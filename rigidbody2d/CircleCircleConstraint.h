...
public:
  static bool isActive(const Vector2s &x0, const Vector2s &x1, const scalar &r0,
                       const scalar &r1);

  CircleCircleConstraint(const unsigned sphere_idx0, const unsigned sphere_idx1,
                         const Vector2s &n, const Vector2s &p, const scalar &r0,
                         const scalar &r1);
  ...
  // const VectorXs& vin, const scalar& ndotv, VectorXs& vout, scalar& alpha )
  // const override;
  virtual void evalgradg(const VectorXs &q, const int col, SparseMatrixsc &G,
                         const FlowableSystem &fsys) const override;
  // virtual void computeGeneralizedFrictionDisk( const VectorXs& q, const
  // VectorXs& v, const int start_column, const int num_samples, SparseMatrixsc&
  // D, VectorXs& drel ) const override; virtual void
  // computeSmoothGeneralizedFrictionDisk( const VectorXs& q, const VectorXs& v,
  // const int start_column, SparseMatrixsc& D ) const override;
  virtual void computeGeneralizedFrictionGivenTangentSample(
      const VectorXs &q, const VectorXs &t, const unsigned column,
      SparseMatrixsc &D) const override;
  ...
  // virtual void computeFrictionMask( const int nbodies, VectorXs&
  // friction_mask ) const override;
