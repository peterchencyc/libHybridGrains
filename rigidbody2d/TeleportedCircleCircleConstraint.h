...
public:
  TeleportedCircleCircleConstraint(const unsigned idx0, const unsigned idx1,
                                   const Vector2s &x0, const Vector2s &x1,
                                   const scalar &r0, const scalar &r1,
                                   const Vector2s &delta0,
                                   const Vector2s &delta1,
  ...
  virtual void evalgradg(const VectorXs &q, const int col, SparseMatrixsc &G,
                         const FlowableSystem &fsys) const final override;
  virtual int impactStencilSize() const final override;
  virtual void
  getSimulatedBodyIndices(std::pair<int, int> &bodies) const final override;
  ...
  virtual void evalH(const VectorXs &q, const MatrixXXsc &basis, MatrixXXsc &H0,
                     MatrixXXsc &H1) const final override;
  ...
  virtual bool
  conservesAngularMomentumUnderImpactAndFriction() const final override;
  virtual std::string name() const override;

  // For binary force output
  // TODO: We might want to change the output format to accept two points to
  // account for portals
  virtual void
...
private:
  virtual void computeContactBasis(const VectorXs &q, const VectorXs &v,
                                   MatrixXXsc &basis) const final override;
