...
BodyBodyConstraint::BodyBodyConstraint(const unsigned idx0, const unsigned idx1,
                                       const Vector2s &p, const Vector2s &n,
                                       const VectorXs &q)
    : m_idx0(idx0), m_idx1(idx1), m_n(n), m_r0(p - q.segment<2>(3 * m_idx0)),
...
scalar BodyBodyConstraint::evalNdotV(const VectorXs &q,
                                     const VectorXs &v) const {
  return m_n.dot(computeRelativeVelocity(q, v));
}

int BodyBodyConstraint::impactStencilSize() const { return 6; }

int BodyBodyConstraint::frictionStencilSize() const { return 6; }

void BodyBodyConstraint::getSimulatedBodyIndices(
...
void BodyBodyConstraint::evalH(const VectorXs &q, const MatrixXXsc &basis,
...
void BodyBodyConstraint::computeContactBasis(const VectorXs &q,
                                             const VectorXs &v,
...
bool BodyBodyConstraint::conservesTranslationalMomentum() const { return true; }

bool BodyBodyConstraint::conservesAngularMomentumUnderImpact() const {
  return true;
}

bool BodyBodyConstraint::conservesAngularMomentumUnderImpactAndFriction()
    const {
  return true;
}

std::string BodyBodyConstraint::name() const { return "body_body"; }

VectorXs BodyBodyConstraint::computeRelativeVelocity(const VectorXs &q,
                                                     const VectorXs &v) const {
  assert(v.size() % 3 == 0);
...
void BodyBodyConstraint::setBodyIndex0(const unsigned idx) { m_idx0 = idx; }

void BodyBodyConstraint::setBodyIndex1(const unsigned idx) { m_idx1 = idx; }

VectorXs
BodyBodyConstraint::computeKinematicRelativeVelocity(const VectorXs &q,
                                                     const VectorXs &v) const {
  // No kinematic contribution
  return VectorXs::Zero(2);
}

void BodyBodyConstraint::getWorldSpaceContactPoint(
...
void BodyBodyConstraint::getWorldSpaceContactNormal(
    const VectorXs &q, VectorXs &contact_normal) const {
  contact_normal = m_n;
}

void BodyBodyConstraint::computeGeneralizedFrictionGivenTangentSample(
  ...
  // Effect on center of mass of body 0
  D.insert(3 * m_idx0 + 0, column) = t.x();
  D.insert(3 * m_idx0 + 1, column) = t.y();
  // Effect on orientation of body 0
  D.insert(3 * m_idx0 + 2, column) = MathUtilities::cross(m_r0, t);

  // Effect on center of mass of body 1
  D.insert(3 * m_idx1 + 0, column) = -t.x();
  D.insert(3 * m_idx1 + 1, column) = -t.y();
  // Effect on orientation of body 1
  D.insert(3 * m_idx1 + 2, column) = -MathUtilities::cross(m_r1, t);
}

void BodyBodyConstraint::evalgradg(const VectorXs &q, const int col,
                                   SparseMatrixsc &G,
  ...
  // Effect on orientation of body 0
  G.insert(3 * m_idx0 + 2, col) = MathUtilities::cross(m_r0, m_n);

  // Effect on center of mass of body 1
  G.insert(3 * m_idx1 + 0, col) = -m_n.x();
  G.insert(3 * m_idx1 + 1, col) = -m_n.y();
  // Effect on orientation of body 1
  G.insert(3 * m_idx1 + 2, col) = -MathUtilities::cross(m_r1, m_n);
}
