...
BoxGeometry::BoxGeometry(const Vector2s &r) : m_r(r) {
...
void BoxGeometry::computeCollisionAABB(const Vector2s &x0, const scalar &theta0,
                                       const Vector2s &x1, const scalar &theta1,
                                       Array2s &min, Array2s &max) const {
  this->AABB(x1, theta1, min, max);
}

scalar BoxGeometry::computeArea() const { return 4.0 * m_r.x() * m_r.y(); }

void BoxGeometry::AABB(const Vector2s &x, const scalar &theta, Array2s &min,
                       Array2s &max) const {
  const Array2s extents{Eigen::Rotation2D<scalar>(theta).matrix().cwiseAbs() *
                        m_r};
  min = x.array() - extents;
  max = x.array() + extents;
}

void BoxGeometry::massAndInertia(const scalar &density, scalar &m,
                                 scalar &I) const {
...
const Vector2s &BoxGeometry::r() const { return m_r; }
