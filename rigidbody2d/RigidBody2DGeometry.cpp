...
void RigidBody2DGeometry::computeAABB(const Vector2s &x, const scalar &theta,
                                      Array2s &min, Array2s &max) const {
  AABB(x, theta, min, max);
  assert((min < max).all());
}

void RigidBody2DGeometry::computeMassAndInertia(const scalar &density,
                                                scalar &m, scalar &I) const {
  assert(density > 0.0);
