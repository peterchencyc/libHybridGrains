...
GLCircleRenderer2D::GLCircleRenderer2D(const unsigned num_points_single_half)
    : m_x_crds(num_points_single_half), m_y_crds(num_points_single_half) {
  assert(num_points_single_half >= 3);
  // Sample the circle at equal intervals
  const GLdouble dtheta{MathDefines::PI<GLdouble>() /
                        GLdouble(num_points_single_half - 1)};
  m_x_crds(0) = 0.0;
...
void GLCircleRenderer2D::renderCircle(
    const Eigen::Matrix<GLdouble, 3, 1> &color) const {
...
void GLCircleRenderer2D::renderSolidCircle(
    const Eigen::Matrix<GLdouble, 3, 1> &color) const {
  glPushAttrib(GL_COLOR);
...
void GLCircleRenderer2D::renderCutCircle(
    const Eigen::Matrix<GLdouble, 3, 1> &color) const {
  glPushAttrib(GL_COLOR);
  ...
  // Draw the second half of the circle
  glColor3d(color.x() + 0.1, color.y() + 0.1, color.z() + 0.1);
  glPushMatrix();
  glRotated(180.0, 0.0, 0.0, 1.0);
  glBegin(GL_POLYGON);
  for (int pnt_num = 0; pnt_num < m_x_crds.size(); ++pnt_num) {
...
void GLCircleRenderer2D::renderCircleOutline(
    const Eigen::Matrix<GLdouble, 3, 1> &color) const {
  glPushAttrib(GL_COLOR);
