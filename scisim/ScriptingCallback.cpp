...
// TODO: Don't check if the name is empty, instead have a subclass that simply
// does nothing
...
void ScriptingCallback::restitutionCoefficientCallback(
...
void ScriptingCallback::frictionCoefficientCallback(
...
void ScriptingCallback::startOfStepCallback(const unsigned next_iteration,
...
void ScriptingCallback::endOfStepCallback(const unsigned next_iteration,
                                          const Rational<std::intmax_t> &dt) {
  if (name().empty()) {
    return;
  }
