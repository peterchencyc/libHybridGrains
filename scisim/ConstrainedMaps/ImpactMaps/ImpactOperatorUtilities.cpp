...
void ImpactOperatorUtilities::computeN(
    const FlowableSystem &fsys,
    const std::vector<std::unique_ptr<Constraint>> &V, const VectorXs &q,
    SparseMatrixsc &N) {
...
void ImpactOperatorUtilities::computeLCPQPLinearTerm(
    const SparseMatrixsc &N, const VectorXs &nrel, const VectorXs &CoR,
    const VectorXs &v0, const VectorXs &v0F, VectorXs &linear_term) {
  assert(v0F.size() == v0.size());
  assert(N.rows() == v0.size());
  assert(N.cols() == nrel.size());
  assert(CoR.size() == nrel.size());
  assert((CoR.array() >= 0.0).all());
  assert((CoR.array() <= 1.0).all());

  linear_term = N.transpose() * v0F + nrel +
                (CoR.array() * (N.transpose() * v0 + nrel).array()).matrix();

// If the CoR is constant, double check against the simplified solution
#ifndef NDEBUG
  if ((CoR.array() == CoR(0)).all()) {
    const VectorXs linear_term_single_cor =
        N.transpose() * (CoR(0) * v0 + v0F) + (1.0 + CoR(0)) * nrel;
    assert((linear_term - linear_term_single_cor).lpNorm<Eigen::Infinity>() <=
           1.0e-6);
  }
#endif
}

void ImpactOperatorUtilities::evalKinematicRelativeVelocityN(
    const VectorXs &q,
