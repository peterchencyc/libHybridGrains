...
// TODO: This class has accumulated up lots of redundant code. Pare down to the
// minimum and cleanup.
...
public:
  ...
  void computeBasis(const VectorXs &q, const VectorXs &v,
                    MatrixXXsc &basis) const;

  // Computes the 'forcing' term as needed by So-bogus to handle restitution and
  // collisions with kinematically scripted boundaries
  void computeForcingTerm(const VectorXs &q, const VectorXs &v,
  ...
  static void evalKinematicRelVelGivenBases(
  ...
  // Performs a pairwise response and adds response into alpha; used by pairwise
  // response methods (e.g. Gauss-Seidel, Jacobi)
  virtual void resolveImpact(const scalar &CoR, const SparseMatrixsc &M,
  ...
  // Adds this constraint to a column of a sparse matrix. Used when forming: N^T
  // M^-1 N
  ...
  virtual void
  computeGeneralizedFrictionDisk(const VectorXs &q, const VectorXs &v,
                                 const int start_column, const int num_samples,
                                 SparseMatrixsc &D, VectorXs &drel) const;

  virtual void computeGeneralizedFrictionGivenTangentSample(
  ...
  // Indices of bodies whose degrees of freedom are stored in the global
  // configuration. Will return a body index in the second entry for kinematic
  // bodies, -1 in the second entry for static geometry.
  ...
  virtual void getWorldSpaceContactPoint(const VectorXs &q,
                                         VectorXs &contact_point) const;
  virtual void getWorldSpaceContactNormal(const VectorXs &q,
  ...
  computeLeverArms(const VectorXs &q) const;
