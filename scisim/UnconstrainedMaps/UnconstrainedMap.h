...
public:
  ...
  // iteration: Iteration we are stepping to (for example, 3 means this step is
  // from time 3 * dt to 4 * dt) dt: Timestep q1: Result of map acting on
  // (q0,v0) v1: Result of map acting on (q0,v0)
  virtual void flow(const VectorXs &q0, const VectorXs &v0,
                    FlowableSystem &fsys, const unsigned iteration,
                    const scalar &dt, VectorXs &q1, VectorXs &v1) = 0;
