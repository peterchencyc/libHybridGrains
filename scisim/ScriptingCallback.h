...
public:
  ...
  void restitutionCoefficientCallback(
      const std::vector<std::unique_ptr<Constraint>> &active_set,
      VectorXs &cor);

  void frictionCoefficientCallback(
  ...
  void startOfStepCallback(const unsigned next_iteration,
                           const Rational<std::intmax_t> &dt);

  void endOfStepCallback(const unsigned next_iteration,
                         const Rational<std::intmax_t> &dt);
...
private:
  ...
  virtual void startOfStep(const unsigned next_iteration,
                           const Rational<std::intmax_t> &dt) = 0;
