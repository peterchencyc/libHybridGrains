...
void GLWidget::generateRenderers( const std::vector<std::unique_ptr<RigidBody2DGeometry>>& geometry )
{
  ...
  for( const std::unique_ptr<RigidBody2DGeometry>& geo : geometry )
  {
    switch( geo->type() )
    {
      ...
      {
        const AnnulusGeometry& annulus{ static_cast<AnnulusGeometry&>( *geo ) };
        m_body_renderers.emplace_back( new AnnulusGeometryRenderer{ annulus, m_circle_renderer } );
        break;
      }
...
void GLWidget::updateRenderers( const std::vector<std::unique_ptr<RigidBody2DGeometry>>& geometry )
{
  ...
  for( unsigned new_idx = 0; new_idx < geometry.size(); ++new_idx )
  {
    switch( geometry[new_idx]->type() )
    {
      case RigidBody2DGeometryType::ANNULUS:
      {
        std::cerr << "ANNULUS not handled in GLWidget::updateRenderers" << std::endl;
...
bool GLWidget::openScene( const QString& xml_scene_file_name, const bool& render_on_load, unsigned& fps, bool& render_at_fps, bool& lock_camera )
{
  ...
  const bool loaded_successfully{ RigidBody2DSceneParser::parseXMLSceneFile( xml_scene_file_name.toStdString(), scripting_callback, new_state, new_unconstrained_map, dt_string, dt, end_time, new_impact_operator, new_imap, CoR, new_friction_solver, mu, new_impact_friction_map, spatial_grid_cell_scale, reduce_bandwidth, camera_settings ) };
  ...
  m_reduce_bandwidth = reduce_bandwidth;
  ...
  assert( ( new_unconstrained_map != nullptr && new_impact_operator == nullptr && new_friction_solver == nullptr && new_impact_friction_map == nullptr && new_imap == nullptr ) ||
         ...
         ( new_unconstrained_map != nullptr && new_impact_operator == nullptr && new_friction_solver != nullptr && new_impact_friction_map != nullptr && new_imap == nullptr ) || (new_impact_friction_map != nullptr && new_impact_friction_map->name() == "penalty_impact_friction_map") );
  ...
  m_sim.grid().setCellScale( spatial_grid_cell_scale );
  ...
  std::cout << "Total simulated discrete mass: " << m_sim.getState().totalSimulatedMass() << std::endl;
...
void GLWidget::stepSystem()
{
  ...
  if( m_if_map != nullptr && m_if_map->name() == "penalty_impact_friction_map" )
  {
    static NullUnconstrainedMap null_map;
    static NullFrictionSolver null_friction_solver;
    m_sim.flow( m_scripting, next_iter, m_dt, null_map, m_CoR, m_mu, null_friction_solver, *m_if_map, m_reduce_bandwidth );
  }
  ...
  {
    m_sim.flow( m_scripting, next_iter, m_dt, *m_unconstrained_map, *m_impact_operator, m_CoR, *m_imap, m_reduce_bandwidth );
  }
  else if( m_unconstrained_map != nullptr && m_impact_operator == nullptr && m_imap == nullptr && m_friction_solver != nullptr && m_if_map != nullptr )
  {
    m_sim.flow( m_scripting, next_iter, m_dt, *m_unconstrained_map, m_CoR, m_mu, *m_friction_solver, *m_if_map, m_reduce_bandwidth );
  }
  else
  {
...
{
  std::cout << "Saving H5 state of time " << std::fixed << std::setprecision( m_display_precision ) << m_iteration * scalar( m_dt ) << " to " << file_name.toStdString() << std::endl;
...
void GLWidget::printBodyState() const
{
  const RigidBody2DState& state = m_sim.getState();

  for( const std::unique_ptr<RigidBody2DGeometry>& geo : state.geometry() )
  {
    if( geo->type() == RigidBody2DGeometryType::CIRCLE )
    {
      const CircleGeometry& cirlce = *static_cast<CircleGeometry*>( geo.get() );
      std::cout << "<geometry type=\"circle\" r=\"" << std::setprecision(std::numeric_limits<scalar>::max_digits10) << cirlce.r() << "\"/>" << std::endl;
    }
    else if( geo->type() == RigidBody2DGeometryType::BOX )
    {
      const BoxGeometry& box = *static_cast<BoxGeometry*>( geo.get() );
      std::cout << "<geometry type=\"box\" r=\"" << std::setprecision(std::numeric_limits<scalar>::max_digits10) << box.r().x() << " " << std::setprecision(std::numeric_limits<scalar>::max_digits10) << box.r().y() << "\"/>" << std::endl;
    }
    else
    {
      std::cerr << "Error, unrecognized geo type in state printing." << std::endl;
    }
  }

  for( unsigned bdyIdx = 0; bdyIdx < state.nbodies(); bdyIdx++ )
  {
    const scalar x = state.q()( 3 * bdyIdx + 0 );
    const scalar y = state.q()( 3 * bdyIdx + 1 );
    const scalar theta = state.q()( 3 * bdyIdx + 2 );
    const scalar vx = state.v()( 3 * bdyIdx + 0 );
    const scalar vy = state.v()( 3 * bdyIdx + 1 );
    const scalar omega = state.v()( 3 * bdyIdx + 2 );
    const int geoidx = state.geometryIndices()[bdyIdx];
    const scalar rho = state.m( bdyIdx ) / state.geometry()[geoidx]->computeArea();
    const int fixed = state.fixed( bdyIdx );
    std::cout << "<rigid_body x=\"" << x << " " << y << "\" theta=\"" << theta << "\" v=\"" << vx << " " << vy << "\" omega=\"" << omega << "\" rho=\"" << rho << "\" geo_idx=\"" << geoidx << "\" fixed=\"" << fixed << "\"/>" << std::endl;
  }
}
...
void GLWidget::paintSystem() const
{
  ...
  {
    const std::vector<RigidBody2DStaticDrum>& drums = m_sim.state().drums();
    for( const RigidBody2DStaticDrum& drum : drums )
    {
      glPushMatrix();
      glTranslated( GLdouble( drum.x().x() ), GLdouble( drum.x().y() ), GLdouble( 0.0 ) );
      glScaled( GLdouble( drum.r() ), GLdouble( drum.r() ), GLdouble( 1.0 ) );
      const scalar theta_degrees{ 180.0 * drum.theta() / MathDefines::PI<scalar>() };
      glRotated( theta_degrees, 0.0, 0.0, 1.0 );
      m_circle_renderer.renderSolidCircle( Eigen::Matrix<GLdouble,3,1>(0.3, 0.3, 0.3) );
      glPopMatrix();
    }
  }
  glPopAttrib();
    ...
    for( const PlanarPortal& planar_portal : planar_portals )
    {
      for( unsigned bdy_idx = 0; bdy_idx < nbodies; ++bdy_idx )
      {
        ...
        if( planar_portal.aabbTouchesPortal( min, max, intersecting_index ) )
        {
          ...
          {
            m_body_renderers[ m_sim.state().geometryIndices()(bdy_idx) ]->renderTeleported( Vector3s{ 0.5, 0.5, 0.5 } );
          }
          glPopMatrix();
        }
      }
